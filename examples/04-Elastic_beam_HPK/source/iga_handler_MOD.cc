...
IgaHandler<dim,spacedim>::IgaHandler (const std::vector<std::vector<double> > &knot_vectors,
  ...
  dh_sys (tria),
  map_fe(NULL),
  local_dof_indices(fe_sys.dofs_per_cell),
  ...
  local_quad(QIterated<dim>(QTrapez<1>(), 30))
{
  ...
  for (typename DoFHandler<dim, spacedim>::active_cell_iterator
       cell = dh_sys.begin_active(); cell != dh_sys.end(); ++cell, ++el)
    {
...
void IgaHandler<dim,spacedim>::assemble_global_extractor()
{
  ...
  DynamicSparsityPattern cp(dh_sys.n_dofs(),n_d);

  std::vector<unsigned int> dofs(fe_sys.dofs_per_cell);

  for (typename DoFHandler<dim, spacedim>::active_cell_iterator
       cell = dh_sys.begin_active(); cell!=dh_sys.end(); ++cell)
    for (unsigned int j=0; j<fe_sys.dofs_per_cell; ++j)
      for (unsigned int i=0; i<fe_sys.dofs_per_cell; ++i)
        {
...
IgaHandler<dim,spacedim>::transform_vector_into_fe_space(
  ...
  Vector<double> &src)
{
  AssertDimension(dh_sys.n_dofs(), dst.size()); // XXX
  ...
  Vector<double> &src)
{
  ...
  AssertDimension(dh_sys.n_dofs(), src.size()); // XXX
...
{
  AssertDimension(fe_sys.dofs_per_cell, dst.size());  // XXX
  AssertDimension(fe_sys.dofs_per_cell, src.size()); // XXX
...
IgaHandler<dim,spacedim>::transform_cell_matrix_into_bspline_space(
  ...
  typename DoFHandler<dim>::active_cell_iterator &cell)
{
  AssertDimension(fe_sys.dofs_per_cell, dst.m());
  AssertDimension(fe_sys.dofs_per_cell, dst.n());
  AssertDimension(fe_sys.dofs_per_cell, src.m());
  AssertDimension(fe_sys.dofs_per_cell, src.n());

  FullMatrix<double> cell_bspline_matrix_buffer(fe_sys.dofs_per_cell, fe_sys.dofs_per_cell);
  ...
  AffineConstraints<double> &constraints)
{
  ...
  Functions::ConstantFunction<spacedim> boundary_funct_fake(1,2);
  fake_boundary[0] = &boundary_funct_fake;

  std::map<types::global_dof_index,double> fake_boundary_values;
std::cout << "\n [B01] " << std::endl;
  VectorTools::project_boundary_values (dh_sys,   // <- XXX dh -> dh_sys [?]
                                        fake_boundary,
                                        q,
                                        fake_boundary_values);
std::cout << "\n [B02] " << std::endl;
  Vector<double> fake_boundary_values_vector(dh_sys.n_dofs());   // <- XXX dh -> dh_sys [?]
...
std::cout << "\n [B1] " << std::endl;
  VectorTools::project_boundary_values (dh_sys,   // <- XXX dh -> dh_sys [?]
                                        ...
                                        boundary_values);
std::cout << "\n [B2] " << std::endl;
  Vector<double> boundary_values_vector(dh_sys.n_dofs());   // <- XXX dh -> dh_sys [?]
std::cout << "\n [B3] " << std::endl;
...
IgaHandler<dim,spacedim>::distribute_local_to_global(
  ...
  AffineConstraints<double> &constraints)
{
  AssertDimension(fe_sys.dofs_per_cell, cell_matrix.m());
  AssertDimension(fe_sys.dofs_per_cell, cell_matrix.n());
  AssertDimension(fe_sys.dofs_per_cell, cell_rhs.size());

  FullMatrix<double> cell_bspline_matrix(fe_sys.dofs_per_cell, fe_sys.dofs_per_cell);
  Vector<double> bspline_cell_rhs(fe_sys.dofs_per_cell);
